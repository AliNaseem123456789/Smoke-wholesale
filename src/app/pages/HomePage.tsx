...
// --- MODAL COMPONENT (Using Tailwind 4) ---
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      ...
      try {
        ...
        setFeaturedProducts(mapWithImages(json.data.featured).slice(0, 15));
        setNewArrivals(mapWithImages(json.data.newArrivals).slice(0, 10));
        setBestSellers(mapWithImages(json.data.bestSellers).slice(0, 10));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
      {products.map(product => (
        ...
        >
          ...
          >
            <img
              src={product.imageUrls[0] || "/product-placeholder.png"}
              alt={product.title}
